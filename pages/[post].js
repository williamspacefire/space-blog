	...
	return (
		<>
			<Header>
				...
				<link
					rel='canonical'
					href={`https://compilado.xyz${permalink}`}
				/>
				...
				<meta
					property='og:url'
					content={`https://compilado.xyz${permalink}`}
				/>
							...
							<Typography className={classes.post_time}>
								{`${date.getDate()}/${
									date.getMonth() < 10
										? '0' + date.getMonth()
										: date.getMonth()
								}/${date.getFullYear()} ás ${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}`}
							</Typography>
							<div className={classes.post_author}>
								{
									<Avatar>
										<Img
											...
											quality={100}
											alt={author}
											src='/v1612101129/avatar/williamspacefire_xuhvy6.jpg'
											width={40}
											height={40}
										/>
									</Avatar>
				...
				{content ? (
					<div className={classes.post_content}>
						...
						<SharePost
							classes={classes}
							title={title}
							url={`https://compilado.xyz${permalink}`}
							twitter={twitter}
						/>
